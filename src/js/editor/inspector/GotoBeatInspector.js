...
define(function(require) {
    ...
    var GotoBeatInspector = function (goto, view) {
        this.goto = goto;
        this.view = view;

        this.container = $('<div>');
        this.container.append('<h2>Go To</h2>');
        this.container.append($('<p><label>Beat: <select id="inspector-goto"></select></p>'));
    ...
    GotoBeatInspector.prototype.show = function() {
    ...
    GotoBeatInspector.prototype.loadValues = function() {
        var Editor = req('editor/Editor');

        // beats
        this.container.find('#inspector-goto').append($('<option>').attr('value', '').text(''));
        var beats = Editor.instance.beatStores;
        for ( var id in beats ) {
            var beat = beats[id];
            var name = beat.get('beat').name;
            this.container.find('#inspector-goto').append($('<option>').attr('value', name).text(name));
        }
